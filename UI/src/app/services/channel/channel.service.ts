...
export class ChannelService {
    ...
    getSubscriberCount(channelId: string): Observable<number> {
        return this.http.post<{ subscribers: number }>(API.GetSubscribers(), { channel: channelId }).pipe(
            map(data => data.subscribers)
        );
    }

    getSubscriptionState(channelId: string): Observable<boolean> {
        return this.http.post<{ isSubscribed: boolean }>(API.GetSubscriptionState(), { channel: channelId! }).pipe(
            map(data => data.isSubscribed)
        );
    }

    subscribeToChannel(channelId: string): Observable<boolean> {
        return this.http.post<{ success: boolean }>(API.PostSubscription(), { channel: channelId }).pipe(
            map(response => response.success)
        );
    }

    unsubscribeFromChannel(channelId: string): Observable<string> {
        return this.http.delete<{ message: string }>(API.DeleteSubscription(channelId)).pipe(
